...
class ConditionNode extends LimitFactorsNode {
    ...
    @Override
    public Optional<Float> getFactor(Branch branch, Branch.Side side, CurrentLimits.TemporaryLimit limit, Contingency contingency) {

        if (condition.matches(branch, side, limit, contingency)) {
            return super.getFactor(branch, side, limit, contingency);
        }
